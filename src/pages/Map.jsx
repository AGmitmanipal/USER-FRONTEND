...
import { parseDateTimeLocal, toDateTimeLocalString } from "../utils/dateUtils";
// 🔐 Mapbox token (do not hardcode secrets in source control)
...
const Map = () => {
  ...
  const handleReserve = async () => {
    ...
    if (!toTime) return alert("Select end time");

    // Reserve = "check-in now" (backend blocks future reservations)
    const start = new Date();
    const end = parseDateTimeLocal(toTime);
    if (!end || Number.isNaN(end.getTime())) return alert("Invalid end time");
    if (end.getTime() <= start.getTime()) return alert("End time must be after now.");
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(data.message || "Parking Failed");
      }
    } catch (err) {
      console.error(err);
      alert("Server Error");
      ...
      <div className="bg-white p-4 shadow-md z-10 flex flex-col md:flex-row justify-between items-center border-b gap-4">
        ...
        <div className="flex items-center gap-2">
          <Link to="/my-reservations" className="bg-gray-100 text-gray-700 hover:bg-gray-200 px-4 py-2 rounded font-bold">
            My Bookings And Parkings
          </Link>
          <button
            onClick={() => {
              // Reserve starts now; show it (read-only) in the modal.
              const now = new Date();
              const localNowISO = toDateTimeLocalString(now);
              setFromTime(localNowISO);
              setShowModal(true);
            }}
            disabled={!canBook}
        ...
        showModal && (
          <div className="absolute inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={() => setShowModal(false)}>
            <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-sm" onClick={e => e.stopPropagation()}>
              <h2 className="text-xl font-bold mb-4">Park Spot</h2>
              <div className="mb-4">
