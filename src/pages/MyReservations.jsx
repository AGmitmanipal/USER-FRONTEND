      ...
      <div className="max-w-3xl mx-auto">
        ...
        {!loading && !error && reservations.length === 0 && (
          <div className="text-center p-10 bg-white rounded-xl shadow-sm border border-dashed border-gray-300">
            <p className="text-gray-500">No bookings or parkings found.</p>
          </div>
        )}
            ...
            return (
              <div key={r._id} className="bg-white p-4 rounded-xl shadow border border-gray-100 flex flex-col md:flex-row justify-between items-start md:items-center">
                <div className="mb-4 md:mb-0">
                  ...
                  <div className={`mt-2 inline-block px-2 py-1 rounded text-xs font-bold uppercase ${r.status === 'reserved' ? 'bg-yellow-100 text-yellow-700' :
                    r.status === 'booked' ? 'bg-blue-100 text-blue-700' :
                      'bg-gray-100 text-gray-500'
                    }`}>
                    {r.status === 'booked' ? 'Booked' : r.status === 'reserved' ? 'Active' : r.status}
                  </div>
                </div>
