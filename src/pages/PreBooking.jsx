...
import { parseDateTimeLocal, datetimeLocalToUTC } from "../utils/dateUtils";
...
export default function PreBooking() {
  ...
  const handleBook = async () => {
    ...
    const selectedStart = parseDateTimeLocal(fromTime);
    if (!selectedStart || selectedStart.getTime() <= Date.now()) {
      return setMsg({ type: 'error', text: "Cannot book or park for a past time." });
    }
    ...
    try {
      ...
      const res = await fetch(`${RESERVATIONS_BASE}/prebook`, {
        ...
        body: JSON.stringify({
          ...
          fromTime: datetimeLocalToUTC(fromTime),
          toTime: datetimeLocalToUTC(toTime)
        })
      });
    ...
    <div className="min-h-screen p-6 bg-gradient-to-br from-purple-600 via-indigo-600 to-pink-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white/10 border border-white/30 rounded-xl p-6 shadow-xl backdrop-blur-md h-fit">
          <h2 className="text-xl font-bold text-white mb-4">Book a Spot</h2>
        ...
        <div className="bg-white/10 border border-white/30 rounded-xl p-6 shadow-xl backdrop-blur-md">
          <h2 className="text-xl font-bold text-white mb-4">My Active Bookings And Parkings</h2>
